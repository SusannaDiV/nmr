...
def open_mnova(mnova_path: Path) -> subprocess.Popen:
    logging.info("Opening MestreNova in Conda environment")
    ...
    process = subprocess.Popen(["powershell", "-Command", command], creationflags=subprocess.CREATE_NEW_PROCESS_GROUP)
    
    time.sleep(2.5)
    return process

# Example usage
mnova_path = Path("C:/Program Files/Mestrelab Research S.L/MestReNova/MestReNova.exe")
...
def run_simulations(
    ...
    for i in range(len(smiles_df)):
        try:
            ...
            logging.error(f"KeyError: 'Smiles' column is missing in the DataFrame at index {i}")
            continue
...
@click.option(
    "--mnova_path",
    type=click.Path(exists=True, path_type=Path),
    default=Path("C:/Program Files/Mestrelab Research S.L/MestReNova/MestReNova.exe"),
    help="Path to the MestreNova executable.",
)
@click.option(
    "--script_path",
    type=click.Path(exists=True, path_type=Path),
    default=Path("C:/path/to/your/scripts"),
...
def main(
    ...
    # Read the CSV file without using the first column as index
    smiles_data = pd.read_csv(smiles_csv, header=None, names=["index", "Smiles"])
    
    # Drop the 'index' column to match expected format
    smiles_data = smiles_data.drop(columns=["index"])
